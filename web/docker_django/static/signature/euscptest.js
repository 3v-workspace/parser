...
var URL_GET_CERTIFICATES = "/static/signature/data/CACertificates.Test.p7b?version=1.0.18";
var URL_CAS = "/static/signature/data/CAs.Test.json?version=1.0.18";
var URL_XML_HTTP_PROXY_SERVICE = "/sign/server/proxyhandler";
...
//-----------------------------------------------------------------------------
	...
	// метод для обробки кліку по кнопці 'Зчитати' приватний ключ, ЦСК і Пароль
	readPrivateKeyButtonClick: function() {
	    // ховаємо форму
	    $("#iit").hide();
	    // показуємо напис 'Опрацьовується'
	    $("#inprogress").show();
	    $("#key-error-form").hide();
	    // записуємо пороль у змінну `passwordTextField`
...
//				euSignTest.clearPrivateKeyCertificatesList();
        ...
        // Чекаємо 0.5 секунди перед тим як почати формувати .xml файл, знімати з нього Геш, тощо.
        setTimeout(function () {
            if (euSign.IsPrivateKeyReaded()) {
                console.log('private readed');
...
//                window.location.replace("/");
                ...
                console.log('private did not readed');
                $("#inprogress").hide();
                $("#iit").show();
                $('#MainPageMenuPKeyPage').show();
                $("#key-error-form").show();
                console.log('private did not readed end');
            }
        }, 500);
	},
...
//-----------------------------------------------------------------------------
    ...
    signHashCustom: function () {
        var ppszHash = '';
        var ppbHash = '';
        var hash = '';
        var ownCert = euSignTest.getOwnCertificate(
				EU_CERT_KEY_TYPE_DSTU4145, EU_KEY_USAGE_DIGITAL_SIGNATURE);
        var id_service_request = $('#id_test_service').val();
        var eservice_type = $('#eservice_type').val();
        var apps = $('#apps').val();
        ...
        $.ajax({async: false,
                type: 'POST',
                dataType: "json",
                url: '/sign/get_file_hash',
                data: {"csrfmiddlewaretoken": $('input[name="csrfmiddlewaretoken"]').val(),
		               "certificate": ownCert,
                       "id_service_request": id_service_request,
                       "eservice_type": eservice_type,
                       "apps": apps
		               },
		        traditional: true
                })
        .done(function(data) {
            if ( data["status"] == "ok" ) {
                ppszHash = data["ppszHash"];
                ppbHash = data["ppbHash"];
//                id_service_request = data["id_service_request"];
        ...
        hash = ppszHash ? ppszHash : ppbHash;
//        alert('hash: ' + hash);
        // Підписання гешу
        sign_hash = euSign.SignHash(hash, true);
//		alert('sign_hash: ' + sign_hash);
		$.ajax({
		    async: false,
		    type: 'POST',
		    dataType: "json",
		    url: '/sign/send_signed_file_hash',
		    data: {"sign_hash": sign_hash,
		    ...
		    if ( data['status'] == 'ok' ) {
		        var pSignInfo = data['pSignInfo'];
		        var url_of_xml = data["xml_url"];
		        var hash_url = data["hash_url"];
		        var service_requests_link = data["service_requests_link"];
//		        alert('Info of customer from server: '+ data['pSignInfo'] + typeof(data['pSignInfo']));
		        var innerUl = '<h4><i class="icon fa fa-check"></i> Передано!</h4>';

		        innerUl += '<ul class="list-group">';
				// link to .xml file
				innerUl += '<li class="list-group-item"><span class="font-weight-bold">Посилання на сформований xml: </span>'
				         + '<a href="' + url_of_xml + '">'
				         + id_service_request + '.xml</a></li>';
				// link ti .hash file
				innerUl += '<li class="list-group-item "><span class="font-weight-bold">Посилання на Геш файлу: </span>'
				         + '<a href="' + hash_url + '">'
				         + id_service_request + '.hash</a></li>';

		        for (var key in pSignInfo) {
                    // check if the property/key is defined in the object itself, not in parent
                    if (pSignInfo.hasOwnProperty(key)) {
//                        console.log(key, pSignInfo[key]);
                        if ( typeof(pSignInfo[key]) == 'object' ) {
                            var innerInnerLi = '';

                            for ( var innerKey in pSignInfo[key] ) {
                                innerInnerLi += innerKey + ': ' + pSignInfo[key][innerKey] + ', ';
                            }
                            innerUl += '<li class="list-group-item"><span class="font-weight-bold">' + key
                                    + '</span>: ' + innerInnerLi + '</li>';
                            innerInnerLi = '';
                        } else {
                            innerUl += '<li class="list-group-item"><span class="font-weight-bold">' + key
                                    + '</span>: ' + data['pSignInfo'][key] + '</li>';
                        }

                    }
                }
                innerUl += '</ul>';
                innerUl += '<h3 class="mt-3"><a href="/eservice/my_eservices/">Мої замовлені послуги</a></h3>';
//                document.getElementById('sign-hash-info').innerHTML = innerUl;
                $('.wrap_iit').prepend(innerUl);
                //////////////
//		        var innerUl = 'Ваші дані успішно сформовані!\n';
//				innerUl += 'Дані:\n';
//
//		        for (var key in pSignInfo) {
//                    // check if the property/key is defined in the object itself, not in parent
...
//                                innerInnerLi += innerKey + ': ' + pSignInfo[key][innerKey] + ', \n';
//                            }
//                            innerUl += key + ': ' + innerInnerLi + '\n';
//                            innerInnerLi = '';
//                        } else {
//                            innerUl += key + ': ' + data['pSignInfo'][key] + '\n';
//                        }
//
//                    }
//                }
//                var url_of_xml = data["xml_url"];
//                innerUl += "Посилання на Ваш файл " + url_of_xml;
//                alert(innerUl);
		    } else {
//		        $('.service-msg').append(data["error"]).show();
                var service_requests_link = data["service_requests_link"];
                var html = '<p>Помилка:</p><p>' + data["error"] + '</p>';
                html += '<h3 class="mt-3"><a href="' + service_requests_link + '">Повернутись до списку моделей</a></h3>';
                $('.card-wrapper').prepend(html);
		    }

        });

//        var info = euSign.VerifyHash(ppszHash ? ppszHash : ppbHash, sign_hash);
//// start Verification signedHash via js
//		setStatus('підпис данних');
//		var message = "Info of customer from JS\nПідпис успішно перевірено";
//
...
//        setStatus('');
//        alert(message);
// end Verification signedHash via js
	},
	verifyData: function () {
		var data = document.getElementById('DataToSignTextEdit').value + "";
