...
from nadra.backend.forms import EServiceBackendFormNadra
...
def create(request, created_by_ip):
    if True:
        if request.method == "POST":
            form = EServiceBackendFormNadra(request.POST, request.FILES)
            ...
            if form.is_valid():
                ...
                Log(eservice_id=e_service.id, action="Послугу було створено"+request.user.first_name,
                    user=request.user).write()

                # "/backend/eservice/update/"+str(last_element+1)+"/2"
                return redirect(reverse("update_eservice",
                                        args=(last_element+1, 2)))

            else:
                return render(request, "nadra/backend/create.html", {
                    "form": form,
                })
        else:
            form = EServiceBackendFormNadra()
...
def view(request, eservice_id, helper):
    ...
    return render(request, "nadra/backend/view.html", {
        "e_service": e_service,
        "approvers": approvers,
        "bosses": bosses,
...
def update(request, eservice_id, created_by_ip, step, helper):
    ...
    if step == 1:
        if request.method == "POST":
            form = EServiceBackendFormNadra(request.POST, request.FILES, instance=e_service)
            # fill a model from a POST
            if form.is_valid():
                e_service = form.save(commit=False)
                e_service.created_by_id = request.user.id
                ...
                return redirect("/backend/eservice/update/"+str(e_service.id)+"/3")
            else:
                return render(request, "nadra/backend/update_step1.html", {"form": form})
        else:
            form = EServiceBackendFormNadra(instance=e_service)
            return render(request, "nadra/backend/update_step1.html", {
                "form": form,
                "e_service": e_service,
        ...
        bosses = User.objects.filter(groups__name="Погоджувачі")
        return render(request, "nadra/backend/update_step2.html", {
            "bosses": bosses,
            "e_service": e_service,
            "helper": helper,
        ...
        return render(request, "nadra/backend/update_step4.html", {
            "e_service": e_service,
            "helper": helper,
